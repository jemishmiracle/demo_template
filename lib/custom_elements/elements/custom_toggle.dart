...
class CustomToggle extends StatefulWidget {
  ...
  /// Button type of [CustomToggleType] i.e, android, ios, custom, sqaure
...
class _CustomToggleState extends State<CustomToggle> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) => Stack(
    children: <Widget>[
      Container(
        height: widget.type == CustomToggleType.android ? 25 : 30,
        width: widget.type == CustomToggleType.android ? 40 : 50,
      ),
      Positioned(
        top: 5,
        child: InkWell(
          borderRadius: widget.type == CustomToggleType.square
              ? const BorderRadius.all(Radius.circular(0))
              : widget.borderRadius ??
              const BorderRadius.all(Radius.circular(20)),
          onTap: onStatusChange,
          child: Container(
            width: widget.type == CustomToggleType.ios ? 45 : 36,
            height: widget.type == CustomToggleType.ios ? 25 : 15,
            decoration: BoxDecoration(
                color: isOn
                    ? widget.enabledTrackColor ?? CustomColors.primary
                    : widget.disabledTrackColor ?? CustomColors.grey,
                borderRadius: widget.type == CustomToggleType.square
                    ? const BorderRadius.all(Radius.circular(0))
                    : widget.borderRadius ??
                    const BorderRadius.all(Radius.circular(20))),
            child: Padding(
              padding: widget.type == CustomToggleType.ios
                  ? const EdgeInsets.only(left: 3.5, right: 3.5, top: 5.4)
                  : const EdgeInsets.only(left: 3, right: 3, top: 3.4),
              child: isOn
                  ? Text(
                  widget.enabledText ??
                      (widget.type == CustomToggleType.custom ? 'ON' : ''),
                  style: widget.enabledTextStyle ??
                      (widget.type == CustomToggleType.ios
                          ? const TextStyle(
                  ...
                  : Text(
                widget.disabledText ??
                    (widget.type == CustomToggleType.custom ? 'OFF' : ''),
      ...
      Positioned(
        top: widget.type == CustomToggleType.ios ? 7.5 : 3,
        left: widget.type == CustomToggleType.ios ? 2 : 0,
        child: InkWell(
          onTap: onStatusChange,
          child: SlideTransition(
            ...
            child: Container(
              ...
              width: 20,
              decoration: BoxDecoration(
                shape: widget.type == CustomToggleType.square
                    ? BoxShape.rectangle
